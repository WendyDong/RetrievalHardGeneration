...
class GoogleNet_Model(object):
    def __init__(self, model_dir='./lib/pretrain_model/', pooling_type=''):
    ...
    def variables_dict(self):
        ...
        if self.pooling_type == "gem":
            variables["Pooling/gem/p"] = tf.get_variable(name='Pooling/gem/p', shape=[1], initializer=tf.constant_initializer(2.9))
    ...
    def forward(self, x):
        ...
        if self.pooling_type == "":
            ...
            p = self.var_dict["Pooling/gem/p"]
            nets = tf.pow(tf.nn.avg_pool(tf.pow(tf.clip_by_value(nets, clip_value_min=self.eps, clip_value_max=float('inf')), p), ksize=[1, 7, 7, 1],
                              strides=[1, 1, 1, 1], padding='VALID'), 1. / p)
        elif self.pooling_type == "mac":
            nets = tf.nn.max_pool(nets, ksize=[1, 7, 7, 1],
                                  strides=[1, 1, 1, 1], padding='VALID')
        elif self.pooling_type == "rmac":
            ovr = 0.4  # desired overlap of neighboring regions
            steps = tf.constant([2, 3, 4, 5, 6, 7])  # possible regions for the long dimension
            L = 3
            W = 7
            H = 7
            ...
            b = (max(H, W) - w) / (steps - 1)
            idx = tf.argmin(tf.abs(((w ** 2 - w * b) / w ** 2) - ovr), 0)
            ...
            vt = tf.nn.max_pool(nets, ksize=[1, 7, 7, 1],
                                  strides=[1, 1, 1, 1], padding='VALID')
            vnor = tf.norm(vt, ord=2, axis=3, keepdims=True) + self.eps
            vnor = tf.tile(vnor, [1, 1, 1,1024])
            v = vt / vnor
            for l in range(1, L + 1):
                wl = tf.floor(2 * w / (l + 1))
                wl_int = math.floor(2 * w / (l + 1))
                wl2 = tf.floor(wl / 2 - 1)
                ...
                if l + Wd == 1:
                    ...
                    b = (W - wl) / (l + Wd - 1)
                cenW = tf.floor(wl2 + tf.convert_to_tensor(np.arange(l - 1 + Wd + 1), dtype=tf.float32) * b) - wl2  # center coordinates
                if l + Hd == 1:
                    b = 0
                else:
                    b = (H - wl) / (l + Hd - 1)
                cenH = tf.floor(wl2 + tf.convert_to_tensor(np.arange(l - 1 + Hd + 1), dtype=tf.float32) * b) - wl2  # center coordinates
                for i_ in range(l - 1 + Hd + 1):
                    for j_ in range(l - 1 + Wd + 1):
                        if wl_int == 0:
                            continue
                        # R = nets[:, :, (int(cenH[i_]) + tf.convert_to_tensor(np.arange(wl), dtype=tf.float32)).tolist(), :]
                        # R = R[:, :, :, (int(j_) + tf.convert_to_tensor(np.arange(wl), dtype=tf.float32)).tolist()]
                        R = tf.map_fn(fn=lambda x: tf.slice(x, begin=[tf.cast(cenH[i_], tf.int32), 0, 0],
                                     size=[wl_int, x.shape[1], x.shape[2]]), elems=nets)
                        R = tf.map_fn(fn=lambda x: tf.slice(x, begin=[0, tf.cast(cenW[j_], tf.int32), 0],
                                     size=[x.shape[0], wl_int, x.shape[2]]), elems=R)
                        vt = tf.nn.max_pool(R, ksize=[1, R.shape[1], R.shape[2], 1],
                                  strides=[1, 1, 1, 1], padding='VALID')
                        vtt = tf.norm(vt, ord=2, axis=3, keepdims=True) + self.eps
                        vtt = tf.tile(vtt, [1, 1, 1, 1024])
                        vt = vt / vtt
                        v += vt
            nets=v
