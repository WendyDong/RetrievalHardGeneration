...
from lib import HDML
from FLAGS import *
from lib import Two_Stage_Model
"""
...
def all_in(loss_vector):
    mean, var = tf.nn.moments(loss_vector, axes=0, keepdims=False)
...
def triplet_loss(anchor, positive, negative, margin=1.6, hard_ori=False, HardOrNot=None, idx=None):
    ...
    if hard_ori:
        loss = tf.multiply(loss, HardOrNot)
    if idx!=None:
        idx = tf.Print(idx, [tf.reduce_mean(idx)*4032])
        loss = tf.multiply(loss, idx)
    return tf.reduce_sum(loss)
def triplet_loss_fan(anchor, positive, negative, margin, hard_ori=False, HardOrNot=None):
    d_ap = tf.reduce_sum(tf.square(anchor - positive), axis=1)
    d_an = tf.reduce_sum(tf.square(anchor - negative), axis=1)
    loss = tf.maximum(-d_ap + d_an + margin, 0.)
    if hard_ori:
...
def two_stage_npair_loss(labels, embedding, reg_lambda, half_batch_size, equal_shape=True):
    embedding_anchor = tf.slice(
        input_=embedding, begin=[0, 0], size=[int(FLAGS.batch_size / 2), int(FLAGS.embedding_size)])
    if equal_shape:
        embedding_positive = tf.slice(
            input_=embedding, begin=[int(FLAGS.batch_size), 0],
            size=[int(FLAGS.batch_size / 2), int(FLAGS.embedding_size)]
        )
    else:
        embedding_positive = tf.slice(
            input_=embedding, begin=[int(FLAGS.batch_size), 0],
...
def new_npair_loss(labels, embedding_anchor, embedding_positive, reg_lambda, half_batch_size, equal_shape=True):
    """
...
def new_npair_loss_fan(labels, embedding_anchor, embedding_positive, reg_lambda, half_batch_size, equal_shape=True):
    """
    N-pair loss used in HDML, the inputs are constructed into N/2 N/2+1 pairs
    :param labels: A 1-d tensor of size [batch_size], which presents the sparse label of the embedding
    :param embedding_anchor: A 4-d tensor of size [batch_size/2, H, W, C], the embedding of anchor
    ...
    for i in range(half_batch_size):
        ...
        pos_label = tf.reshape(label_pos, [half_batch_size, 1])
        labels_remapped = tf.to_float(
            tf.not_equal(anc_label, tf.transpose(pos_label))
        )
        labels_remapped /= tf.reduce_sum(labels_remapped, 1, keepdims=True)
...
def npairs_loss(labels, embeddings_anchor, embeddings_positive,
                reg_lambda=3e-3, print_losses=False, hard_ori=False ,HardOrNot=None, idx=None):
    ...
    if idx==None:
        ...
        idx = tf.Print(idx, [tf.reduce_mean(idx)])
        xent_loss = tf.multiply(xent_loss,idx)
        xent_loss = math_ops.reduce_mean(xent_loss, name='xentropy')
    if hard_ori:
...
def npairs_loss_fan(labels, embeddings_anchor, embeddings_positive,
                reg_lambda=3e-3, print_losses=False, hard_ori=False, HardOrNot=None):
    # pylint: enable=line-too-long
    # Add the regularizer on the embedding.
    reg_anchor = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embeddings_anchor), 1))
    ...
    return l2loss + xent_loss
def npairs_loss_fan_new(labels, embeddings_anchor, embeddings_positive,
                reg_lambda=3e-3, print_losses=False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_anchor=embeddings_anchor
    embedding_positive=tf.tile(embeddings_positive , [half_batch_size, 1])
    reg_anchor = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_anchor), 1))
    reg_positive = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_positive), 1))
    ...
    if equal_shape:
        ...
        pos_tile = embedding_positive
    anc = tf.split(embedding_anchor, half_batch_size, axis=0) #1*len
    pos = tf.split(pos_tile, half_batch_size, axis=0) #half_batch* len
    label2 = tf.split(labels, 2, axis=0)
    label_anc = tf.reshape(label2[0], [half_batch_size, 1])
    label_pos = tf.reshape(label2[1], [half_batch_size, 1])
    label_anc = tf.split(label_anc, half_batch_size, axis=0)
    ...
    for i in range(half_batch_size):
        ...
        anc_label = tf.reshape(label_anc[i], [1, 1])
        pos_label = tf.reshape(label_pos, [half_batch_size, 1])
        labels_remapped1 = tf.to_float(
            tf.equal(anc_label, tf.transpose(pos_label))
        )
        labels_remapped2 = tf.to_float(
            tf.not_equal(anc_label, tf.transpose(pos_label))
        )
        # 归一化这个值，不要太大太小，可能存在问题！！！！！！！！！！！！！
        # similarity_matrix=tf.nn.softmax(similarity_matrix)
        # exp_result = tf.exp(similarity_matrix)
        # exp_result_fenmu=labels_remapped2/exp_result
        # idx = tf.where(exp_result_fenmu > 0)
        # exp_result_fenmu = tf.gather_nd(exp_result_fenmu, idx)
        # exp_result_fenzi=labels_remapped1*exp_result
        # idx = tf.where(exp_result_fenzi > 0)
        # exp_result_fenzi = tf.gather_nd(exp_result_fenzi, idx)
        #
        # dot_operation = tf.multiply(exp_result_fenzi, exp_result_fenmu)  # tf.matmul dot
        # dot_operation = tf.log(tf.reduce_sum(dot_operation)+1)
        # xent_loss.append(dot_operation)

        ##换一种做法，尝试不先softmax,softmax之后这个值会变得特别小
        beijian = tf.gather_nd(similarity_matrix, tf.where(labels_remapped2 >0 ))
        jian = tf.gather_nd(similarity_matrix, tf.where(labels_remapped1 > 0))
        # exp_result = tf.softplus()
        zhongjian=tf.reduce_sum(tf.exp(jian-beijian))
        zh=tf.Print(zhongjian,[zhongjian,jian,beijian,similarity_matrix])
        exp_result = tf.log(zh+1)
        xent_loss.append(exp_result)

    xent_loss = tf.reduce_mean(xent_loss, name='xentrop')
    r_loss = tf.cond(tf.is_nan(xent_loss + l2loss), lambda: tf.constant(0.), lambda: xent_loss + l2loss)
    return r_loss

def npairs_loss_fan_tri(labels, embeddings_anchor, embeddings_positive,margin=1.4,
                reg_lambda=3e-3, print_losses = False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_anchor = embeddings_anchor
    embedding_positive = embeddings_positive
    embedding_negative = embedding_positive
    reg_anchor = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_anchor), 1))
    reg_positive = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_positive), 1))
    ...
    anc_tile = tf.reshape(tf.tile(embedding_anchor, [1, int(FLAGS.batch_size / 2)]), [-1, int(FLAGS.embedding_size_fan)])
    pos_tile = tf.reshape(tf.tile(embedding_positive, [1, int(FLAGS.batch_size / 2)]), [-1, int(FLAGS.embedding_size_fan)])
    neg_tile = tf.tile(embedding_negative, [int(FLAGS.batch_size / 2), 1])
    disap = Two_Stage_Model.distance(pos_tile, neg_tile)
    disap = tf.reshape(disap, [int(FLAGS.batch_size / 2)*int(FLAGS.batch_size / 2)])
    ind = tf.where(disap > 0.0)
    ...
    xent_loss = triplet_loss_fan(anc_tile, pos_tile, neg_tile, margin, hard_ori, HardOrNot)
    r_loss = tf.cond(tf.is_nan(xent_loss + l2loss), lambda: tf.constant(0.), lambda: xent_loss + l2loss)
    # if hard_ori:
    #     r_loss = tf.multiply(r_loss, HardOrNot)
    return r_loss


def npairs_loss_fan_tri2(labels, embeddings, margin=1.4,
                reg_lambda=3e-3, print_losses = False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_size = embeddings.shape[1].value
    embedding_anchor = tf.slice(
                input_=embeddings, begin=[0, 0], size=[half_batch_size, embedding_size])
    embedding_positive = tf.slice(
                input_=embeddings, begin=[half_batch_size, 0], size=[half_batch_size, embedding_size])
    if equal_shape:
        embedding_negative = tf.slice(
            input_=embeddings, begin=[FLAGS.batch_size, 0],
            size=[int(half_batch_size), embedding_size])
        embedding_negative = tf.tile(embedding_negative, [int(half_batch_size), 1])
    else:
        embedding_negative = tf.slice(
                input_=embeddings, begin=[FLAGS.batch_size, 0],
                size=[int(np.square(half_batch_size)), embedding_size])
    reg_anchor = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_anchor), 1))
    reg_positive = math_ops.reduce_mean(
        math_ops.reduce_sum(math_ops.square(embedding_positive), 1))
...
def npairs_loss_fan_cluster_durpair(labels, embeddings_anchor, embeddings_positive,margin=1.4,
                reg_lambda=3e-3, print_losses = False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_anchor = embeddings_anchor
    embedding_positive = embeddings_positive
    reg_anchor = math_ops.reduce_mean(
    ...
    l2loss = math_ops.multiply(
        0.25 * reg_lambda, reg_anchor + reg_positive, name='l2loss')
    mean_anchor = tf.reduce_mean(embedding_anchor, axis=0)
    mean_positive = tf.reduce_mean(embedding_positive, axis=0)
    mean_center = tf.reduce_mean(mean_positive + mean_anchor)
    dis_anchor = tf.reduce_sum(tf.sqrt(tf.reduce_sum(tf.square(embedding_anchor - mean_center), axis=1)))
    dis_positive = tf.reduce_sum(tf.sqrt(tf.reduce_sum(tf.square(embedding_positive - mean_center), axis=1)))
    # aims to make it smaller
    dis_nei = (dis_anchor + dis_positive) * 0.5
    # aims to make it bigger
    dis_pair = tf.reduce_sum(tf.sqrt(tf.reduce_sum(tf.square(embedding_anchor - embedding_positive), axis=1)))
    xent_loss = dis_nei / dis_pair
    # xent_loss = tf.Print(xent_loss, [xent_loss], message='xent_loss is ' , first_n=None, summarize=None, name=None)
    r_loss = tf.cond(tf.is_nan(xent_loss + l2loss), lambda: tf.constant(0.), lambda: xent_loss + l2loss)
    if hard_ori:
        r_loss = tf.multiply(r_loss, HardOrNot)
    return r_loss

def npairs_loss_fan_cluster(labels, embeddings_anchor, embeddings_positive,margin=1.4,
                reg_lambda=3e-3, print_losses = False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_anchor = embeddings_anchor
    embedding_positive = embeddings_positive
    reg_anchor = math_ops.reduce_mean(
    ...
    mean_anchor = tf.reduce_mean(embedding_anchor, axis=0)
    dis_anchor = tf.reduce_sum(tf.sqrt(tf.reduce_sum(tf.square(embedding_anchor - mean_anchor), axis=1)))
    mean_positive = tf.reduce_mean(embedding_positive, axis=0)
    dis_positive = tf.reduce_sum(tf.sqrt(tf.reduce_sum(tf.square(embedding_positive - mean_positive), axis=1)))
    dis_p_a = tf.sqrt(tf.reduce_sum(tf.square(mean_anchor-mean_positive)))

    xent_loss = (dis_anchor + dis_positive)*0.5 - dis_p_a
    # xent_loss = tf.Print(xent_loss, [xent_loss], message='xent_loss is ' , first_n=None, summarize=None, name=None)
    # xent_loss = tf.reduce_mean(xent_loss, name='xentrop')
    r_loss = tf.cond(tf.is_nan(xent_loss + l2loss), lambda: tf.constant(0.), lambda: xent_loss + l2loss)
    if hard_ori:
...
def npairs_loss_fan_cluster_div(labels, embeddings_anchor, embeddings_positive,margin=1.4,
                reg_lambda=3e-3, print_losses = False, hard_ori=False, HardOrNot=None, equal_shape=False, half_batch_size=64):
    embedding_anchor = embeddings_anchor
    embedding_positive = embeddings_positive
    reg_anchor = math_ops.reduce_mean(
...
def Loss(embedding, label, _lossType="Softmax", loss_l2_reg=FLAGS.loss_l2_reg, hard_ori=False, idx=None):
    ...
    _Loss = 0
    HardOrNot = HDML.HardOrNot(_lossType, embedding)
    if _lossType == "Softmax":
        ...
        print("Use NpairLoss")
        _Loss = npairs_loss(label_positive, embedding_anchor, embedding_positive, reg_lambda=loss_l2_reg, hard_ori=hard_ori, HardOrNot=HardOrNot, idx=idx)
        ...
        _Loss = triplet_loss(anchor, positive, negative, hard_ori=hard_ori, HardOrNot=HardOrNot, idx=idx)
        ...
        _Loss = new_npair_loss(
            ...
            half_batch_size=int(FLAGS.batch_size / 2), equal_shape=True)

    return _Loss


def Loss_fan(embedding, label, _lossType="Softmax", loss_l2_reg=FLAGS.loss_l2_reg, param=1.3, hard_ori=False, equal_shape=False):
    # if
    embedding_split = tf.split(embedding, 2, axis=0)
    label_split = tf.split(label, 2, axis=0)
    embedding_anchor = embedding_split[0]
    ...
    _Loss = 0
    # HardOrNot = HDML.HardOrNot(_lossType, embedding)
    if _lossType == "NpairLoss":
        # _Loss = npairs_loss_fan(label_positive, embedding_anchor, embedding_positive,half_batch_size=int(FLAGS.batch_size/2),
        #                         reg_lambda=loss_l2_reg,hard_ori=hard_ori, HardOrNot=HardOrNot)
        # _Loss = npairs_loss_fan_new(label, embedding_anchor, embedding_positive,half_batch_size=int(FLAGS.batch_size/2),
        #                         reg_lambda=loss_l2_reg, hard_ori=hard_ori, HardOrNot=HardOrNot)
        _Loss = npairs_loss_fan_tri(label, embedding_anchor, embedding_positive,canshu,half_batch_size=int(FLAGS.batch_size/2),
                                    reg_lambda=loss_l2_reg, hard_ori=hard_ori)
        # _Loss = npairs_loss_fan_tri2(label, embedding, canshu,
        #                             half_batch_size=int(FLAGS.batch_size / 2),
        #                             reg_lambda=loss_l2_reg, hard_ori=hard_ori, equal_shape=equal_shape)
        # _Loss = npairs_loss_fan_cluster(label, embedding_anchor, embedding_positive, canshu, half_batch_size=int(FLAGS.batch_size / 2),
        #                             reg_lambda=loss_l2_reg, hard_ori=hard_ori, HardOrNot=HardOrNot)
        # _Loss = npairs_loss_fan_cluster_div(label, embedding_anchor, embedding_positive, canshu,
        #                                 half_batch_size=int(FLAGS.batch_size / 2),
        #                                 reg_lambda=loss_l2_reg, hard_ori=hard_ori, HardOrNot=HardOrNot)
        # _Loss = npairs_loss_fan_cluster_durpair(label, embedding_anchor, embedding_positive, canshu,
        #                                 half_batch_size=int(FLAGS.batch_size / 2),
        #                                 reg_lambda=loss_l2_reg, hard_ori=hard_ori, HardOrNot=HardOrNot)
    elif _lossType == "Triplet":
        print("Use Triplet Loss")
        ...
        positive = embedding3[1]
        negative = embedding3[2]
        _Loss = triplet_loss_fan(anchor, positive, negative, canshu, hard_ori=hard_ori, HardOrNot=None)
    elif _lossType == "New_npairLoss_fan":
        print("Use new NpairLoss fan")
        _Loss = new_npair_loss_fan(
            labels=label, embedding_anchor=embedding_anchor,
            embedding_positive=embedding_positive, reg_lambda=loss_l2_reg,
            half_batch_size=int(FLAGS.batch_size / 2), equal_shape=True)
    else:
